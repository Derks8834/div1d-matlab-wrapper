function  [output,input]= div1dread_v210(path)
% div1dread_v210(path) reads the output div1doutput.txt specified in path
...
cell  = textscan(fid, '   carbon_concentration    = %f','delimiter','\n'); input.carbon_concentration = cell2mat(cell(1));
...
cell  = textscan(fid, '   switch_car_con_prf      = %u','delimiter','\n'); input.switch_car_con_prf = cell2mat(cell(1));
...
input.gas_puf_prf = zeros(Nx,1);
 % read the information line and skip
    ...
    profile_inputs = textscan(fid,' %f %f %f %f',Nx, 'delimiter','\n'); 
    Xdum = cell2mat(profile_inputs(:,1));
    input.car_con_prf = cell2mat(profile_inputs(:,2));
    input.gas_puf_prf = cell2mat(profile_inputs(:,3));
    input.flx_exp_prf = cell2mat(profile_inputs(:,4));
