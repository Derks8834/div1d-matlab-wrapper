function [ox, oy, n] = define_div1d_normal_2t(x,y,w,xcb,ycb,ixp,nwall,wxp_list)
Nx = length(x);
%wxp_list(1:2) = [30, 50]
%wxp = ceil(Nx/10);
...
for i = 1:2
    ...
    if i == 1
    in = [max(1,xp-4*wxp), max(2,xp-2*wxp), min(xp+2*wxp,Nx/2-2), min(xp+4*wxp,Nx/2-1)];
    ...
    in = [max(xp-4*wxp,Nx/2+1),max(xp-2*wxp,Nx/2+2), min(xp+1*wxp,Nx-1) , min(xp+5*wxp,Nx-1)];
    nint(1,1:4) = [n(1,in(1)), -1.5,-1.5, n(1,in(4))];
    nint(2,1:4) = [n(2,in(1)),  1, 1, n(2,in(4))];
    end
    % interp xdir
