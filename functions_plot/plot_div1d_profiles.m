function [h] = plot_div1d_profiles(div1doutput, div1dinput, varargin)
...
plimp = P.plotimpurities; plimp = plimp -1;
Ntime       = P.Ntime;
...
B_field = div1dinput.grid.b_field';
...
%          solps.geom.dspar = repmat(max(solps.geom.dspar),length(solps.geom.dspar(:,1)),1)-solps.geom.dspar;
    ...
    if P.figtight ==0
        ...
        h = 1-y2-y1;
        dy = h/6;
        qpos    = [x1, y1+dy*5,w,dy ];
        tpos    = [x1, y1+dy*4,w,dy ];
        nepos   = [x1, y1+dy*3,w,dy ];
        upos    = [x1, y1+dy*2,w,dy ];
        ndpos   = [x1, y1+dy*1,w,dy ];
        fcpos   = [x1, y1     ,w,dy ];
    ...
    qparline(1) =  plot(Xcb,q_parallel(Ntime,:)/10^6, 'LineWidth', P.LineWidth);
    if P.pxline ==1
    if div1dinput.grid.i_xpoint(1) > 1;  xline(X(div1dinput.grid.i_xpoint(1)),'-'); end
    if div1dinput.grid.i_xpoint(2) > 1;  xline(X(div1dinput.grid.i_xpoint(2)),'-'); end
    end
    ...
    if ~isfig; qlab = ylabel('$q_{\|}$ (MW m$^{-2}$)','FontSize',P.FontSize,'interpreter','latex'); end 
    ...
    if P.figtight ==0
       ...
       if ~isfig; ax.t = axes('Position',tpos, 'FontSize', P.FontSize-1); else; ax.t = fig.Children(5+plimp);  fig.CurrentAxes = ax.t; end; box on;
    end
    if P.hold == 1;  hold on; end
    ...
    if ~isfig; tlab = ylabel('$T$ (eV)','FontSize',P.FontSize,'interpreter','latex'); end
    ...
    if P.figtight ==0
       ...
       if ~isfig; ax.ne = axes('Position',nepos, 'FontSize', P.FontSize-1); else; ax.ne = fig.Children(4+plimp);  fig.CurrentAxes = ax.ne; end; ax.ne; box on;
    end
    if P.hold == 1;  hold on; end
    plot(X,density(Ntime,:), 'LineWidth', P.LineWidth); grid on; hold on;
    if P.pxline == 1
    if div1dinput.grid.i_xpoint(1) > 1;  xline(X(div1dinput.grid.i_xpoint(1)),'-'); end
    if div1dinput.grid.i_xpoint(2) > 1;  xline(X(div1dinput.grid.i_xpoint(2)),'-'); end
    end
    ...
    if ~isfig; nlab = ylabel('$n_{\mathrm{e}}$ (m$^{-3}$)','FontSize',P.FontSize,'interpreter','latex'); end 
    set(gca,'YScale','log')
    ...
    if P.figtight ==0
       ...
       if ~isfig; ax.u = axes('Position',upos, 'FontSize', P.FontSize-1); else; ax.u = fig.Children(3+plimp);  fig.CurrentAxes = ax.u; end; ax.u; box on;
    end
    if P.hold == 1;  hold on; end
    plot(X,velocity(Ntime,:)/10^3, 'LineWidth', P.LineWidth); grid on;
    if P.pxline ==1
    if div1dinput.grid.i_xpoint(1) > 1;  xline(X(div1dinput.grid.i_xpoint(1)),'-'); end
    if div1dinput.grid.i_xpoint(2) > 1;  xline(X(div1dinput.grid.i_xpoint(2)),'-'); end
    end
    ...
    if ~isfig; vlab = ylabel('$v_{\mathrm{\|}}$ (10$^3$ms$^{-1}$)','FontSize',P.FontSize,'interpreter','latex');  end
...
if P.figtight ==0
   ...
   if ~isfig; ax.nd = axes('Position',ndpos, 'FontSize', P.FontSize-1); else; ax.nd = fig.Children(2+plimp);  ax.nd;  fig.CurrentAxes = ax.nd; end; box on;
end
...
if ~isfig; dlab = ylabel('$n_{\mathrm{D0}}$ (m$^{-3}$)','FontSize',P.FontSize,'interpreter','latex'); end
set(gca,'YScale','log')
if max(div1dinput.physics.extern_neutral_density) > 0 % plot neutral background
    if P.plotbackground ==1 
    if div1dinput.grid.i_xpoint(1) > 1
        plot(X([1,div1dinput.grid.i_xpoint(1)]),[1 1]*div1dinput.physics.extern_neutral_density(1),...
            '--','color',[0, 0.4470, 0.7410],'LineWidth',P.LineWidth);
        plot(X([1,div1dinput.grid.i_xpoint(1)]),[1 1]*div1dinput.physics.extern_neutral_density(2),...
            '--','color',[0, 0.4470, 0.7410],'LineWidth',P.LineWidth);
    end
    if div1dinput.grid.i_xpoint(2) > 1
        plot(X([div1dinput.grid.i_xpoint(1),div1dinput.grid.i_xpoint(2)]),...
            [1 1]*div1dinput.physics.extern_neutral_density(3),...
            '--','color',[0, 0.4470, 0.7410],'LineWidth',P.LineWidth);
        if P.pxline == 1
        xline(X(div1dinput.grid.i_xpoint(2)), '-',  'X-point','FontSize',P.FontSize-2, 'interpreter','latex',...
            'LabelHorizontalAlignment', 'left')
        end
    end
    
     plot(X([div1dinput.grid.i_xpoint(2),end]),...
            [1 1]*div1dinput.physics.extern_neutral_density(4),'--','color',[0, 0.4470, 0.7410],'LineWidth',P.LineWidth)
     leg(1) = plot(X([div1dinput.grid.i_xpoint(2),end]),...
      [1 1]*div1dinput.physics.extern_neutral_density(5),...
            '--','color',[0, 0.4470, 0.7410],'LineWidth',P.LineWidth);
      legend(leg(1),'$n_{\mathrm{b}}$','FontSize',P.FontSize,'interpreter','latex','location','best'); 
    end
end
if max(P.ylim_b) ~=0;    ylim(P.ylim_b); end
if P.ytic_b ~= 0; yticks(P.ytic_b); end 
if P.flip ==1
    if P.figtight ==0;    xlabel('distance to target (m)','FontSize',P.FontSize-2,'interpreter','latex'); % ,'interpreter','latex'
    else
        if P.plotimpurities ~=0;  xticklabels({''});           xticks(P.xticks); end
    end
    set(gca,'XDir','reverse');
else
    xlabel('distance from upstream (m)','interpreter','latex')
...
xlim(P.xlimits);
if P.ylim_v ~=0;    ylim(P.ylim_v); end
if P.ytic_v ~=0;  yticks(P.ytic_v); end


% carbon fractio
if P.plotimpurities == 1

    if P.figtight ==0
        figure(P.fignum+9);
        position = generalposition + [0 0 0 0 ];
        set(gcf, 'OuterPosition',position);
    else
        if ~isfig; ax.fc = axes('Position',fcpos, 'FontSize', P.FontSize-1); else; ax.fc = fig.Children(1+plimp); end; box on;
    end
    if P.avg == 0
        if isstruct(P.solps)
            if P.flip ==1
                plot(flip([0 max(X)]),[1 1]*median(solps.data.cratio_correct)*100, 'LineWidth', P.LineWidth);
            else
                plot([0 max(X)],[1 1]*median(solps.data.cratio_correct)*100, 'LineWidth', P.LineWidth);
            end
        else
            if P.flip ==1
                plot(flip([0 max(X)]),[1 1]*median(solps.data.cratio_correct(:,1))*100, 'LineWidth', P.LineWidth);
            else
                plot([0 max(X)],[1 1]*median(solps.data.cratio_correct(:,1))*100, 'LineWidth', P.LineWidth);
            end
        end
    end
    if P.cratio ~= 0
        if P.flip ==1
            plot(flip([0 max(X)]),[1 1]*P.cratio*100, 'LineWidth', P.LineWidth);
        else
            plot([0 max(X)],[1 1]*P.cratio*100, 'LineWidth', P.LineWidth);
        end
    end
    hold on;
    if P.avg ==0
        if isstruct(P.solps)
            plot(solps.geom.dspar,solps.cratio_correct(:,1),'--k', 'LineWidth', P.LineWidth);
        end
    else
        if isstruct(P.solps)
            plot(solps.geom.dspar(:,1),solps.data.cratio_correct(:,1)*100,'--k', 'LineWidth', P.LineWidth);
            tmpX = [solps.geom.dspar(1,1); solps.geom.dspar(:,1); flip(solps.geom.dspar(:,1))];
            tmpY = [solps.data.cratio_correct(1,1); solps.data.cratio_correct(:,2);  flip(solps.data.cratio_correct(:,3))]*100;
            patch(tmpX,tmpY,[0 0 0],'FaceAlpha',0.3,'LineStyle','none') % the errorbar
        end
        xlim(P.xlimits);
    end

    if isempty(P.implabel)
        flab = ylabel('$\xi_{\mathrm{Z}}$ (\%)','interpreter','latex','FontSize',P.FontSize); grid on; % 'interpreter','latex',
    else
        tmpstr = strcat('$\xi_{\mathrm{',P.implabel,'}}$ (\%)');
        flab = ylabel(tmpstr,'interpreter','latex','FontSize',P.FontSize); grid on;
    end
    if P.ylim_f ~=0;    ylim(P.ylim_f); end
    if P.ytic_f ~=0;  yticks(P.ytic_f); end
end

% finally plot the xlabel 
if P.flip ==1
     xlabel('distance to target (m)','interpreter','latex','FontSize',P.FontSize); % interpreter','latex',
%      set(gca,'XDir','reverse');
     else
     xlabel('distance from upstream (m)','interpreter','latex')
end   
