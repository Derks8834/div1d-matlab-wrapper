function plotdiv1d_below_v4(output,input,varargin)
% function to plot legacy versions of DIV1D

% Author: Gijs Derks
% E-mail: g.l.derks@differ.nl
% July 2023

%% old version of plotting, meant for older versions, but likely needs adjustments.
D.solps = 0;
D.S2PM = 0;
D.D2PM = 0;
...
plot(X,temperature(Ntime,:), 'LineWidth', P.LineWidth);  grid on;
if isstruct(P.D2PM)
if P.flip ==1
...
if P.hold == 1;  hold on; end
line(1) = plot(X,density(Ntime,:), 'LineWidth', P.LineWidth); grid on;
if isstruct(P.D2PM)
if P.flip ==1
...
if P.hold == 1;  hold on; end
plot(X,velocity(Ntime,:), 'LineWidth', P.LineWidth); grid on;
if isstruct(P.solps);   hold on;
